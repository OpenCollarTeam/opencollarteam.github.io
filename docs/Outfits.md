...
**Note for Users of Restrained Love Viewer** Due to a difference in how hidden folders are handled, the stock Outfits script works with RLVa and not RLV. RLVa is used by [Firestorm Viewer](https://www.firestormviewer.org/) and several other RLV enabled third party viewers.  RLV is used by Marine Kelley's [Restrained Love Viewer](http://www.erestraints.com/realrestraint/).  To use Outfits with Restrained Love Viewer you will have to replace oc_outfits with [oc_outfits (RLV)](https://github.com/OpenCollarTeam/OpenCollar/blob/master/src/Apps/oc_outfits%20(RLV).lsl).  Then follow the instructions below, except when spelling the Outfits and Core folders in your inventory, eliminate the period in front of the name:  `outfts` and `core` in place of `.outfits` and `.core`.  Unfortunately this means your Outfits will appear in the Folders menu.
# Setting up Outfits
